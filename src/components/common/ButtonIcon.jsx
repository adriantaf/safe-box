...
function ButtonIcon({ className = "", children, outline = false, onClick = null, disabled = false }) {
  return (
    !outline
      ? <button onClick={ onClick } disabled={ disabled } type="button" className={ `${className} ${disabled ? 'bg-blue-400 dark:bg-blue-900 text-neutral-300 cursor-not-allowed' : 'text-white hover:shadow-md bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'} focus:outline-none font-medium rounded-md text-sm px-3 py-2 text-center inline-flex items-center` }>
        { children }
      </button>
